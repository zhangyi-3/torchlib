...
PROCESS_NAME = "changeme"


def main(args, params):
  # data = dataset(args.data_dir, transform=dset.ToTensor())
  # val_data = dataset(args.val_data_dir, transform=dset.ToTensor())
      ...
      for step, batch in enumerate(dataloader):
        ...
        if args.viz_step > 0 and global_step % args.viz_step == 0:
          ...
          for val_batch in val_dataloader:
            ...
            vizdata = th.cat((lowspp, target, output, diff), 0)
            vizdata = (vizdata-mini)/(maxi-mini)
            imgs = np.power(np.clip(vizdata.cpu().data, 0, 1), 1.0/2.2)

            image_viz.update(
                imgs,
                caption="Epoch {:.1f} | mse = {:.6f} | {}spp, target, output".format(
                  frac_epoch, val_loss.data[0], batch["0_radiance_direct"].shape[-1]), per_row=lowspp.shape[0])
...
if __name__ == "__main__":
  ...
  parser.add_argument('--params', nargs="*", default=["model=Samples"])
